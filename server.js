...
// --- POLLING: fetch replies from Instantly ---

async function pollInstantly() {
  console.log(`[${new Date().toISOString()}] Polling Instantly for new replies...`);

  try {
    const res = await axios.get("https://api.instantly.ai/api/v1/emails/list", {
      params: {
    ...
    const emails = res.data?.data || res.data || [];

    for (const email of emails) {
      const replyId = email.id || email.email_id;

      if (notifiedReplies.has(replyId)) continue;

      // Filter for positive replies only
      // Instantly labels: 1 = Interested, 2 = Meeting Booked
      const label = email.is_interested || email.label || email.reply_label;
      const isPositive = label === 1 || label === "Interested" || label === "Meeting Booked";
      ...
      const leadEmail = email.from_address || email.lead_email || "Unknown";
      const leadName = email.from_name || leadEmail;
      const fromInbox = email.to_address || email.sending_account || "Unknown inbox";
      const campaignName = email.campaign_name || "Unknown campaign";
      const subject = email.subject || "(no subject)";
      const body = (email.body || email.text || email.preview || "").trim().slice(0, 1000);
...
app.post("/telegram-webhook", async (req, res) => {
  ...
  try {
    await axios.post("https://api.instantly.ai/api/v1/emails/reply", {
      api_key: INSTANTLY_API_KEY,
      reply_to_email_id: context.email_id,
      from_email: context.from_inbox,
