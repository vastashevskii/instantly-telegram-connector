...
async function pollInstantly() {
  ...
  try {
    ...
    for (const email of emails) {
      ...
      const rawBody = typeof email.body === 'string' ? email.body : typeof email.preview === 'string' ? email.preview : JSON.stringify(email.body || email.preview || '');
...
app.listen(PORT, () => {
  ...
  setTimeout(() => {
    pollInstantly().catch(err => console.error('Initial poll error:', err.message));
  }, 2000);
  setInterval(() => {
    pollInstantly().catch(err => console.error('Poll interval error:', err.message));
  }, POLL_INTERVAL_MS);
});
